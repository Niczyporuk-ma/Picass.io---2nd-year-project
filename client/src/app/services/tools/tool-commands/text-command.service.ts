...
export class TextCommandService extends UndoRedoCommand{
  ...
  setTextAttributes
  (fontSize: number, font: string, textArray: string[], alignment: CanvasTextAlign, startingPoint: Vec2, endPoint: Vec2, primaryColor: string, italic: boolean, bold: boolean): void {
  ...
  evaluateAlignment(ctx: CanvasRenderingContext2D, index: number): void {
    if(this.alignment === "left"){
      ctx.fillText(this.textArray[index], this.startingPoint.x, this.startingPoint.y + this.toolStyle.lineWidth + (this.toolStyle.lineWidth * index));
    }
      ...
      ctx.fillText(this.textArray[index], ((this.endPoint.x - this.startingPoint.x)/2) + 
      this.startingPoint.x, this.startingPoint.y + 
      this.toolStyle.lineWidth + 
      (this.toolStyle.lineWidth * index));
    }
  }
